  ...
  const fetchIndices = async () => {
    try {
      ...
      setIndices({
        ...
        niftyit: { value: d['^NSEIT']?.price?.toFixed(2) || '--', change: d['^NSEIT']?.change?.toFixed(2) || '--', percent: d['^NSEIT']?.changePercent?.toFixed(2) || '--', up: (d['^NSEIT']?.change || 0) >= 0 },
      });
    ...
    <div>
      ...
      <div className="two-column">
        ...
        <div className="right-column">
          <div className="section-card"><div className="section-header"><h2>🪙 Cryptocurrency</h2><span className="live-badge">LIVE</span></div><div className="crypto-grid">{crypto.map(c => <div key={c.symbol} className="crypto-item"><div className="crypto-symbol">{c.symbol}</div><div className="crypto-price">₹{c.price}</div><div className={c.up ? 'positive' : 'negative'}>{c.change}%</div></div>)}</div></div>
